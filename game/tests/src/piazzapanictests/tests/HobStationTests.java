...
/**
 * Tests for the frying stations.
 * Hobs only interact with raw patties and cooked patties so no other items need to be used or checked.
 *
...
public class HobStationTests extends MasterTestClass {
    ...
    public void testRemoveItemFromFryer() {  // This also tests if the correct items can be placed on the hob
        ...
        hobStation.GiveItem(new Item(ItemEnum.CookedPatty)); // gives a new raw patty to the hob to check if it can be retrieved when cooked
        hobStation.RetrieveItem(); // Attempts to retrieve a cooked patty from the hob to check if it can be retrieved
        assertNull("There should be no item on the hob station", hobStation.RetrieveItem());
        hobStation.GiveItem(new Item(ItemEnum.Cinder)); // gives a burnt patty to the hob to check if it can be retrieved
    ...
    /**
     * Tests that the hob station can burn a burger if maximum progress for both cooking and interaction is reached.
     *
     * @author Hubert Solecki
     * @date 17/04/2023
     */
    ...
    public void testBurnItemOnHob() {
        ...
        hobStation.GiveItem(new Item(ItemEnum.RawPatty)); // gives a raw patty to the hob to test if it can be burnt
        hobStation.Cook(10);
        hobStation.Interact(); // sets the interaction variable to true and paired with the line below flips the patty
        hobStation.Cook(5);
        hobStation.Cook(10); // cooks the patty to completion for 10 delta times (final time step) to set progress to 0 afterwards
        hobStation.Cook(10); // burns the patty after one time step too many
        Item test = hobStation.RetrieveItem();
        assertEquals("Tests that the retrieved burnt patty from the hob is the same as a cinder burnt item", test, new Item(ItemEnum.Cinder));
    }
    ...
    public void testIncorrectItemOnHob() {
        ...
        Item rawPatty = new Item(ItemEnum.RawPatty); // creates the valid items that can be cooked on the hob for testing
        Item cookedPatty = new Item(ItemEnum.CookedPatty);
        instantiateWorldAndHobsStation(); // create hob station
        for (ItemEnum test : Arrays.asList(ItemEnum.values())) { // creates a list of all items in the game and steps through them to check which can interact with the hob
            Item testing = new Item(test);
            hobStation.GiveItem(testing);
            if (!((testing.equals(rawPatty)) || (testing.equals(cookedPatty)))) { // if the items are not a raw or cooked patty (i.e. items that can't interact with the hob), the current recipe should be null showing they cannot interact with the hob
                assertNull("Current recipe should be null if the item is not on the white list of the hob station and therefore cannot be cooked", hobStation.currentRecipe);
            }
            else {
                assertNotNull("Tests that if there are raw patties or cooked patties, they can be placed and be processed by the hob as they are in the white list", hobStation.currentRecipe);
            }
    ...
    public void testItemRetrievedDuringFryingAndProgress() {
        ...
        Item test = hobStation.RetrieveItem();
        assertNotEquals("The value of the progress of the cooking item should not be 0 when it is removed from the hob", (int)testProgress, 0);
        assertNull("The hob should not contain an item when an in progress item is removed from it", hobStation.RetrieveItem());
        assertNotEquals("The progress of the item should not be 0 when it has been cooking for some time and is removed", test.progress, 0);
        assertEquals("The progress of the item being cooked should be the same before and after it is retrieved from the hob", (int)testProgress, (int)test.progress);
    }
    ...
    public void testUpdateMethodHobStation() {
        ...
        hobStation.Interact();
        assertTrue("The interaction attribute is true when interacted with in the update function", hobStation.GetInteracted());
        assertNotNull("The recipe on the hob station is not null when there is an item on it", hobStation.currentRecipe);
        hobStation.Cook(5);
        hobStation.Update(10);
        Item test = hobStation.RetrieveItem();
        assertFalse("The interaction attribute is set to false after the item is retrieved from the hob", hobStation.GetInteracted());
    }
